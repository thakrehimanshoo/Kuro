...
export default function TimerPage() {
  ...
  return (
    <div className="flex flex-col h-screen bg-black text-white lg:ml-64 xl:ml-72 overflow-hidden">
      {/* Main content - vertically centered */}
      <div className="flex-1 flex flex-col items-center justify-center w-full px-3 xs:px-4 sm:px-6 md:px-8 lg:px-12 max-w-2xl xl:max-w-3xl mx-auto safe-top">
        {/* Pomodoro cycle indicator + Phase label */}
        <div className="text-center mb-2 xs:mb-3 sm:mb-4 md:mb-6">
          {type === 'work' && (
            <div className="mb-1 xs:mb-2 flex items-center justify-center gap-1.5 xs:gap-2 md:gap-3">
              {[1, 2, 3, 4].map((num) => (
                <div
                  key={num}
          ...
          <div className="text-[9px] xs:text-[10px] sm:text-xs md:text-sm opacity-40 uppercase tracking-[0.1em] xs:tracking-[0.15em]">
        ...
        <div className="relative mb-3 xs:mb-4 sm:mb-6 md:mb-8">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center">
              <div className="text-4xl xs:text-5xl sm:text-7xl md:text-8xl lg:text-8xl font-extralight tracking-tighter">
                {formatTime(timeLeft)}
              </div>
              <div className="text-[8px] xs:text-[10px] sm:text-sm md:text-base opacity-30 tracking-wide max-w-[120px] xs:max-w-[160px] sm:max-w-xs md:max-w-sm truncate mx-auto mt-0.5 md:mt-1">
                {getMessage()}
              </div>
            </div>
          </div>
        ...
        {/* Controls */}
        <div className="flex items-center gap-2 xs:gap-3 sm:gap-4 md:gap-5 mb-2 xs:mb-3 sm:mb-4">
          <button
            onClick={reset}
            disabled={status === 'idle' && timeLeft === totalTime}
            className="w-10 h-10 xs:w-11 xs:h-11 sm:w-14 sm:h-14 md:w-14 md:h-14 rounded-full border border-white/10 flex items-center justify-center transition-all duration-200 active:scale-95 hover:border-white/30 disabled:opacity-20"
          >
          ...
          <button
            ...
            className="w-14 h-14 xs:w-16 xs:h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-full border-2 border-white flex items-center justify-center transition-all duration-200 active:scale-95 hover:bg-white/5"
          >
            {status === 'running' ? (
              <PauseIcon className="w-5 h-5 xs:w-6 xs:h-6 sm:w-8 sm:h-8 md:w-10 md:h-10" />
            ) : (
              <PlayIcon className="w-5 h-5 xs:w-6 xs:h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 ml-0.5" />
            )}
          </button>
        ...
        <div className="flex flex-col items-center gap-1.5 xs:gap-2 sm:gap-3">
          ...
          {(status !== 'idle' || timeLeft !== totalTime) && (
            <button
              ...
              className="px-3 py-1.5 text-[9px] xs:text-[10px] sm:text-xs opacity-30 hover:opacity-60 transition-opacity"
            >
              Abandon
            </button>
          )}
        ...
        <div className="text-[9px] xs:text-[10px] sm:text-xs opacity-20 mt-2 xs:mt-3 md:mt-4">
